...
Execution is agentic when an LLM is configured: the agent reasons over the claim limitations, candidate descriptions, and evidence in one tool-use conversation, with a full saved reasoning trace and a resumable conversation for feedback.
...
3. **Start** with `guided_search_start` (`search_type="infringement"`, the patent number, and claims). **Present the plan preview and estimated time up front.**
4. **Collect plan feedback** via `guided_search_feedback` (seeded into the agent's initial guidance); revise until approved.
5. **Execute** with `guided_search_execute` and present the ranked candidates: name, confidence (0-1), and the per-candidate rationale explaining which limitations appear practiced. Show the reasoning via `get_search_trace` when an LLM ran the ranking.
6. **Iterate**: ask which candidates merit deeper analysis, feed verdicts back via `guided_search_feedback` (injected into the SAME resumed agent conversation), and re-execute with refined candidates/evidence.
